...
class Route():
    def __init__(self,points,width, accur = 1):
        self.points = points  # The points to define the street
        self.width = width
        self.accur = accur
        self.routepoints = self.get_route(self.points, self.width , self.accur)
    ...
    def get_route(self,points,width,accur):
        ...
        kpoints = [new_p[0],points[0]]
        numPoint = calculate_length(kpoints)
        # print("numPoint is %r" % numPoint)
        x, y = s.Bezier_Kurve(points=kpoints, nTimes=int(numPoint/accur))
        # x,y = s.Bezier_Kurve(points=[new_p[0],points[0]],nTimes=numPoint/10)
        x = list(x)
        y = list(y)
        y.pop()
        x.pop()
        ...
        for i in range(1,n-1):
            ...
            numPoint = calculate_length(kpoints)
            # print("numPoint is %r" % numPoint)
            x_new, y_new = s.Bezier_Kurve(points=kpoints,nTimes=int(numPoint/accur)) # Ablenkung
            x_new = list(x_new)
            y_new = list(y_new)
            x_new.pop()
            y_new.pop()
            ...
            numPoint = calculate_length(lpoints)
            # print("numPoint is %r" % numPoint)
            x_new, y_new = s.Bezier_Kurve(points=lpoints,nTimes=int(numPoint/accur))
        ...
        kpoints = [points[n-1],new_p[2*(n-2)]]
        numPoint = calculate_length(kpoints)
        # print("numPoint is %r" % numPoint)
        x_final, y_final = s.Bezier_Kurve(points=kpoints, nTimes=int(numPoint/accur))
